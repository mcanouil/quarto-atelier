...
A `website.sidebar` of your own is styled rather than configured: a `style: docked` sidebar is dark-pinned from the `--atelier-sidebar-*` tokens, which default to the navbar ones, while a `style: floating` sidebar keeps Quarto's page-coloured treatment.
